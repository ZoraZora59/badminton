...
| US-4.3 临时球友 Guest | `POST /activities/:id/participants`、`PATCH/DELETE …/participants/:pid` | api.test 加 Guest（isGuest=true，无 openid）；Guest 与 +1 占位均可改名/改本场水平/移除（仅局长；已进对阵不可删） | ✅ |
...
4. **自助签到 + +1 显形**：非局长在活动详情点「我已到场·签到」（可撤销）；局长进签到页，**带 +1 的球友下方自动出现「XX的朋友」占位行**，可点击改名、改本场水平、✕ 移除（移除即把带人者「带 N 人」减 1）；勾人/加临时球友 → 「确认参赛 · 去分组」，**确认人数已含 +1**。
