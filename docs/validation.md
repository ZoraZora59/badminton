...
| US-2.4 首页球局单列表 + 空态 | `GET /activities`（`ActivityVM` 新增 `updatedAt`） | `home` 页**去掉三态筛选栏**，改为单列表（进行中 → 报名中 → 已结束；组内按 `updatedAt` 倒序＝最新有状态变化的在最上面，已结束沉底并画「已结束」分隔线，已取消不进列表）；排序与分隔位置抽到 `frontend/src/utils/activity.ts`，`frontend/test/activity.test.ts` 14 条单测锁定（含缺 `updatedAt` 时的降级）；后端 `api.test` 断言 `updatedAt` 出到 VM 且编辑后前进；卡片右上角标签仍表达状态；v1 不做口令/粘贴框，空态误导文案已移除（改为「分享给球友一起打」） | ✅ |
...
| US-3.3 带朋友 +1 | `POST …/signups {plusOne}` | 占位计数含 +1（capacity 计算）；`activity` 页「+1 带人」**点击弹确认选「带1人/带2人」**。**+1 在签到页自动物化成归属带人者的 Guest 行**（`broughtBySignupId`，幂等），局长现场补名字/本场水平、可移除（移除即回收 `plusOne`）；api.test「E3+」覆盖物化/幂等/改名/移除 | ✅ |
| US-3.4 请假 | `POST …/signups/me/leave` | api.test leave→LEAVE 并触发补位 | ✅ |
| US-3.5 头像墙三类名单 | `GET /activities/:id/signups` | `activity` 页**三区块头像墙**：已报名/候补/请假，各区块头像+昵称+人数，超 8 人折叠 +N。**+1 带人在已报名/候补里按真实身位画虚位影位**（表头人数=身位口径，与卡片「已报名 N」一致）；请假不带 +1 入场 | ✅ |
...
3. **三类名单/报名/候补/+1**：换账号点分享进入 → 报名；把人数填满验证「满员候补」；取消一人看候补自动补位；「+1 带人」弹确认选带1/2人；活动详情看到**已报名/候补/请假三区块头像墙**，且 **+1 在已报名/候补里多出一个虚位影位（「XX带」）**，人数与卡片「已报名 N」一致。
4. **自助签到 + +1 显形**：非局长在活动详情点「我已到场·签到」（可撤销）；局长进签到页，**带 +1 的球友下方自动出现「XX的朋友」占位行**，可点击编辑昵称/性别/本场水平、✕ 移除（移除即把带人者「带 N 人」减 1）；勾人/加临时球友 → 「确认参赛 · 去分组」，**确认人数已含 +1**。
