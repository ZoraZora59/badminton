...
- UI/UX 还原度仍有待补齐，重点看 [`docs/ui-ux-fidelity-handoff-2026-06-25.md`](docs/ui-ux-fidelity-handoff-2026-06-25.md)。

优先缺口：

- 活动分享卡、战报分享卡还没有形成真实预览/生成闭环。
- 发起页仍有 `AA` 相关占位文案，违反 v1 “不碰钱”红线。
- 分组页需要从一次性面板改成更清楚的选人/玩法/设置/预览/微调向导。
- 混双已落地：建局阶段开关（仅双打）+ 引擎按性别强制组队，分组向导沿用建局的玩法/模式/场地/混双设置。
- DevTools 里观察到一次 `appServiceSDKScriptError timeout` 和若干 `scroll-view` 渲染告警，需要真机复验并清理。
